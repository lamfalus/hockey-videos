...
Create `credentials/telegram.json` on the Pi:
```json
{
...
```
(Environment variables `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` work too and
take precedence.)

That's it. The next refresh seeds silently; the one after a new upload posts it.
